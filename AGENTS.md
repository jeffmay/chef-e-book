...
- **Inline-edit rows** — instruction and container rows collapse to a one-line summary and open into an inline editor. Edits live in a local `draft` until "✔︎" (accept); "↩" reverts, or removes the row when it was added during this editing session and never accepted. Collapsed summaries render their head in bold — the container type ("**Bowl** — dry ingredients") and the instruction action ("**Bake** the Flour in the Oven"). Pure helpers live in `components/recipe_editor/drafts.ts`:
...
- Add sections with an optional header (hidden until a `+ Add Section Header` button is clicked, which reveals and focuses the input). The header is an inline editor confirmed with ↩/✔︎ and collapses to its heading text; ↩ restores the header the section had when the editor opened, dropping it entirely when it was added in that pass
- Add/edit ingredients (with measurement editor); each ingredient row in a section shows its own measurement value next to the name (independent of the aggregate Ingredients list). A collapsed row floats an "✎" edit button (to the left of the "−" remove button) in its upper-right corner; the editor swaps "✎" for ↩/✔︎, applies name and amount changes as they are made, and reverts the whole row on ↩
