...
- Every editable row uses "✎" (edit) and "✕" (remove) buttons in left-to-right order, floating in the row's upper-right corner. "✕" is the only remove glyph — never "−" or "×".
- **`EditActions`** (`components/edit_actions/`) — the shared "↩" / "✔︎" pair. Takes `cancelLabel`/`acceptLabel` (the accessible names, which say what is being accepted) plus the two handlers, and an optional `className` for per-row layout. Used by every `RecipeVersionEditor` row and its section headers.
- **`RowActions`** (`components/row_actions/`) — the shared "✎" / "✕" pair, absolutely positioned into the row's upper-right corner (rows reserve the space with `padding-right`). "✎" is omitted when `onEdit` is not given — while the row's editor is already open, or on a row with no separate edit mode.
...
- Add/edit text blocks; like the ingredient rows they collapse to their text and reopen from "✎", writing through as they are typed so ↩ restores the text the block held when its editor opened
