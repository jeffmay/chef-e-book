...
- **`EditActions`** (`components/edit_actions/`) — the shared "↩" / "✔︎" pair. Takes `cancelLabel`/`acceptLabel` (the accessible names, which say what is being accepted) plus the two handlers. It follows the fields it commits inline rather than floating to the right of the row, and its wrapper keeps the two buttons together so they wrap to the next line as one left-aligned unit when the row runs out of width. Used by every `RecipeVersionEditor` row and its section headers.
