...
- **Inline-edit rows** — instruction and container rows collapse to a one-line summary and open into an inline editor. Edits live in a local `draft` until "✔︎" (accept); "↩" reverts, or removes the row when it was added during this editing session and never accepted. Collapsed summaries render their head in bold — the container type ("**Bowl** — dry ingredients") and the instruction action ("**Bake** the Flour in the Oven"). Pure helpers live in `components/recipe_editor/drafts.ts`:
  ...
  - `summarizeInstruction` / `summarizeContainer` — the collapsed one-liner; `instructionSummaryName` + `instructionSummaryDetails` and `containerDisplayName` + `containerSummaryDetails` are the same text split at the bolded head
